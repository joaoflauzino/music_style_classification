...
def read_categories_examples(path: str = "../output/songs.csv") -> dict:
    ...
    for idx, cat in enumerate(categories):
        example = songs[songs['category'] == cat].sample(1).iloc[0]['transformed_without_tags']
...
def get_examples_to_classify(path: str = "../output/songs.csv") -> tuple[dict, dict]:
    ...
    for idx, category in enumerate(categories):
        row = songs[songs['category'] == category].sample(1).iloc[0]
        lyric = row['transformed_without_tags'][:500]
...
if __name__ == "__main__":
    ...
    prompt_sistema = (
                ...
                f"Use como referência os exemplos abaixo, considerando a letra, e o número médio de palavras distintas por gênero (moda)."
