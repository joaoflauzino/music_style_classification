...
def preprocess_tags_lyrics(df: pd.DataFrame) -> pd.Series:
    return df["lyrics"].apply(clean_just_tags)
...
def generate_histograms(df: pd.DataFrame, output_path="../images/histogram.png"):
...
def main():
    ...
    df['transformed_without_tags'] = preprocess_tags_lyrics(df)
